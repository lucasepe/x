...
func (dc *Context) LoadFontFace(path string, dpi, points float64) error {
...
func (dc *Context) LoadFontFaceFromFS(fsys fs.FS, path string, dpi, points float64) error {
	face, err := LoadFontFaceFromFS(fsys, path, dpi, points)
	if err != nil {
		return err
	}
...
func (dc *Context) LoadFontFaceFromBytes(raw []byte, dpi, points float64) error {
	face, err := LoadFontFaceFromBytes(raw, dpi, points)
	if err != nil {
		return err
	}
...
func (dc *Context) FontMetrics() font.Metrics {
	return dc.fontFace.Metrics()
}
